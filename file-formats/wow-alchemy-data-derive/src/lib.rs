...
pub fn wow_header_r_derive(input: TokenStream) -> TokenStream {
    ...
    let expanded = if let Some(version_ty) = &struct_wow_attrs.version {
        ...
        quote! {
            impl #impl_generics wow_alchemy_data::types::VWowHeaderR<#version_ty> for #struct_name #ty_generics #where_clause {
                fn wow_read<R: ::std::io::Read + ::std::io::Seek>(reader: &mut R, version: #version_ty) -> wow_alchemy_data::error::Result<Self> {
                    Ok(#reader_body)
                }
            }
        }
        ...
        quote! {
            impl #impl_generics wow_alchemy_data::types::WowHeaderR for #struct_name #ty_generics #where_clause {
                fn wow_read<R: Read + Seek>(reader: &mut R) -> wow_alchemy_data::error::Result<Self> {
...
pub fn wow_header_w_derive(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics wow_alchemy_data::types::WowHeaderW for #ident #ty_generics #where_clause {
            fn wow_write<W: ::std::io::Write>(&self, writer: &mut W) -> wow_alchemy_data::error::Result<()> {
                #writer_body
            }
...
fn generate_wow_enum_from_value_lines(
    ...
    lines.push(if let Some(has_default) = has_default {
        ...
        quote! {
            _ => {
                return Err(wow_alchemy_data::error::WowDataError::InvalidEnumParsedValue("".into(), "".into()).into());
            }
        }
    });
...
pub fn wow_enum_from_derive(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics TryFrom<#ty> for #enum_name #ty_generics #where_clause {
            type Error = wow_alchemy_data::error::WowDataError;

            fn try_from(value: #ty) -> wow_alchemy_data::error::Result<Self> {
                Ok(match value {
                    #(#from_value_lines)*
                })
            }
